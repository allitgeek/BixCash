...
<body class="bg-neutral-50 min-h-screen pb-32 pt-0 px-0" style="margin: 0;">
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl shadow-lg shadow-blue-900/10 border-t border-gray-200/60 z-50">
        <div class="grid grid-cols-5 max-w-7xl mx-auto">
            ...
            {{-- Profile (Active) --}}
            <a href="{{ route('partner.profile') }}" class="flex flex-col items-center justify-center py-3 px-2 text-white bg-gradient-to-r from-blue-600 to-blue-900 border-t-2 border-blue-500 transition-all duration-200 active:scale-95">
                <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
    ...
    <script>
        ...
        window.RESEND_OTP_URL = "{{ route('partner.bank-details.resend-otp') }}";
    ...
    {{-- OTP Modal Functions (using backend OTP service) --}}
    <script>
        // ============================================
        // OTP MODAL FUNCTIONS
        // ============================================
        ...
        function showOtpModal() {
            ...
            // OTP was already sent by backend when form was submitted
            // Handle different channels
            if (window.IS_UFONE_BYPASS && window.UFONE_OTP_CODE) {
                alert(`Your OTP is: ${window.UFONE_OTP_CODE}\n\nPlease enter this code to continue.`);
                document.getElementById('otpCode').value = window.UFONE_OTP_CODE;
                document.getElementById('verifyBtn').disabled = false;
            } else if (window.OTP_CHANNEL === 'whatsapp') {
                showNotification('Verification code sent to your WhatsApp', 'success');
            } else {
                showNotification('Verification code sent to ' + window.USER_PHONE, 'success');
            }
        ...
        async function resendOtp() {
            ...
            try {
                ...
                const response = await fetch(window.RESEND_OTP_URL, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
        ...
        function showNotification(message, type = 'success') {
            ...
            notification.className = `fixed top-5 right-5 ${bgColor} text-white px-6 py-4 rounded-xl shadow-2xl z-[2000] flex items-center gap-3`;
    ...
    <style>
        ...
        /* Apply animations to header stat cards */
        header .grid > div {
            animation: fadeInUp 0.5s ease-out both;
        }
        header .grid > div:nth-child(1) { animation-delay: 0.1s; }
        header .grid > div:nth-child(2) { animation-delay: 0.2s; }
        header .grid > div:nth-child(3) { animation-delay: 0.3s; }
