...
<body>
    <div class="invoice-container">
                ...
                <tbody>
                    @foreach($ledger->settlements->where('voided_at', null) as $settlement)
                        <tr>
                            ...
                            <td>{{ $settlement->processedByUser->name ?? 'N/A' }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="no-print" style="text-align: center; margin-top: 30px;">
            ...
            <button onclick="window.history.back()" style="background: #6c757d; color: white; border: none; padding: 15px 40px; border-radius: 8px; font-size: 16px; cursor: pointer; margin-left: 10px;">
                ← Back
            </button>
        </div>
    </div>
